	...
	for (let i = 0; i < data.length; i++) {
		let $element = $("<a>")
			.attr({
				...
				href: "loader.html#" + btoa(encodeURIComponent(JSON.stringify([data[i].directory, data[i].image, data[i].name, "sppa"]))),
			})
...
function redirectGame(dir) {
	let a = gamelist.find((x) => x.directory == dir);
	window.location.href =
		"loader.html#" +
		btoa(encodeURIComponent(JSON.stringify([a.directory, a.image, a.name, "sppa"])));
}
