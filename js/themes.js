function customTheme() {
	localStorage.setItem("obsidian.theme", "custom");
...
document.addEventListener("DOMContentLoaded", () => {
	loadTheme();
    if(localStorage.getItem("obsidian.theme") == "custom"){
        document.getElementById("customMenu").style.display = "block";
...
function loadTheme() {
	if (localStorage.getItem("obsidian.theme") == "custom") {
		let theme = localStorage.getItem("obsidian.customTheme");
		if (theme) {
...
function changeTheme(name, value) {
	...
	ogStyle = localStorage.getItem("obsidian.customTheme");
	if (ogStyle) {
		ogStyle = JSON.parse(ogStyle);
		ogStyle[name] = value;
		localStorage.setItem("obsidian.customTheme", JSON.stringify(ogStyle));
		loadTheme();
	} else {
		ogStyle = {};
		ogStyle[name] = value;
